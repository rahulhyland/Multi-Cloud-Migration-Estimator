...
# Supports both new format (report.md) and legacy format (multi-cloud-migration-report-*.md).
LATEST_REPORT=$(ls -t "$REPORTS_DIR"/*/report.md "$REPORTS_DIR"/*/multi-cloud-migration-report-*.md 2>/dev/null | head -1)
...
# Try legacy filename format first: multi-cloud-migration-report-YYYYMMDD-HHMMSS-utc.md
match = re.match(r"multi-cloud-migration-report-(\d{8})-(\d{6})-utc\.md$", name)
if match:
    date_part, time_part = match.groups()
else:
    # New format: report.md inside a timestamped folder multi-cloud-migration-YYYYMMDD-HHMMSS-utc
    folder = path.parent.name
    match = re.match(r"multi-cloud-migration-(\d{8})-(\d{6})-utc$", folder)
    if not match:
...
if is_placeholder_chart(graph_model) and svg_path.exists():
